...
async function fetchNodeInfo(baseApiUrl, fileKey, nodeIdsChunk, accessToken) {
    try {
        const response = await axios_1.default.get(`${baseApiUrl}${fileKey}/nodes?ids=${nodeIdsChunk.join(',')}`, {
            headers: {
        ...
        if (response.status !== 200) {
            logging_1.logger.error('Failed to fetch node info: ' + response.status + ' ' + response.data?.message);
            return null;
        }
        ...
        if ((0, axios_1.isAxiosError)(err)) {
            if (err.response) {
                logging_1.logger.error(`Failed to to fetch node info (${err.code}): ${err.response?.status} ${err.response?.data?.err ?? err.response?.data?.message}`);
            }
            else {
...
async function validateDocs(accessToken, docs) {
